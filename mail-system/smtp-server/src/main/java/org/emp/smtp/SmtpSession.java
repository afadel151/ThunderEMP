 ...
 *   FIX       : HELO → "250 smtp.eoc.dz"
 *               EHLO → multi-line "250-smtp.eoc.dz\r\n250 8BITMIME"
 *
 ...
 *
 * BUG 4 — MAIL FROM regex matches "FROM:  <>" (empty address) (RFC 5321 §4.1.1.2)
    ...
    public void run() {
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                switch (command) {
                    case "EHLO": handleEhlo(argument, true);  break;
                    case "HELO": handleEhlo(argument, false); break;
                    case "MAIL": handleMailFrom(argument);    break;
                    case "RCPT": handleRcptTo(argument);      break;
                    case "DATA": handleData();                 break;
                    case "RSET": handleRset();                 break;   // FIX #7
                    case "NOOP": handleNoop();                 break;   // FIX #8
                    case "VRFY": handleVrfy(argument);         break;   // FIX #9
